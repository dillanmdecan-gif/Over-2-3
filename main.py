...
# Locked to the two target symbols — Phase 1 still collects both
# to derive their calibrated thresholds; Phase 2 trades both.
SURVEY_SYMBOLS = ["R_10", "1HZ10V"]
    ...
    async def run(self) -> dict:
        ...
        while time.time() < deadline:
            ...
            try:
                ...
                # Only stop if deadline has genuinely passed
...
# Shared mutable state for the health server — updated by Phase 2
# without restarting the HTTP server on the same port
...
def start_health_server(traders: List[SymbolTrader], phase: str,
    ...
    # Only start the server once — if already running, state update above is enough
    ...
    class _H(BaseHTTPRequestHandler):
        def do_GET(self):
            ...
            if self.path == "/status":
                ...
                # Human summary
                if phase == "collect":
                    elapsed = time.time() - _health_state["collect_start"]
                    remaining = max(0, COLLECT_SECS - elapsed)
...
async def main():
    ...
    # Health server already running from Phase 1 — update state only
    _health_state["traders"] = traders
