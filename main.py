...
class Config:
    ...
    payout_ratio:  float = 0.95        # Deriv digitover payout
    ...
    null_hyp_window:     int   = 25    # was 50 — shorter window detects R_100 bias sooner
    null_hyp_p_value:    float = 0.10  # was 0.05 — less stringent; entropy layer handles randomness separately

    # ── ENTRY THRESHOLDS ─────────────────────────────────────────────────────
    min_over2_prob:       float = 0.78
    min_over3_prob:       float = 0.78
    min_final_conf:       float = 0.55  # was 0.72 — loosened until NN accumulates 50+ training trades
    min_regime_stability: float = 0.48  # was 0.55 — lowered pending root-cause fix of entropy scale bug
...
class DigitNet:
    ...
    def __init__(self, cfg: Config):
        ...
        # Pre-seed output biases toward known R_100 priors:
        #   P(digit > 2) ≈ 0.70  → logit(0.70) ≈ +0.847
        #   P(digit > 3) ≈ 0.60  → logit(0.60) ≈ +0.405
        # Without this the NN starts at 0.5 and produces weak confidence
        # signals until it has accumulated enough training trades (50+).
        self.b_out  = np.array([0.847, 0.405, 0.0, 0.0])
...
class Bot:
    ...
    async def on_tick(self, price: float):
        ...
        # BUG FIX: pass (1 - composite) so RegimeDetector receives entropy_score
        # (low = clean = good) not raw composite (high = random = bad).
        # Previously passing ent["composite"] caused REGIME_STABLE confidence to
        # compute as max(0.5, 1.0 - 0.50) = 0.50 every tick instead of
        # max(0.5, 1.0 - 0.20) = 0.80, permanently blocking the regime gate.
...
def run_backtest(cfg: Config, n_ticks: int = 6000, seed: int = 42):
    ...
    for dur in DURATIONS:
        ...
        for i, price in enumerate(prices_master):
            ...
            rid, rc = reg.push(price, 1.0 - en["composite"])  # BUG FIX: invert composite
